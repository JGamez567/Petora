...
export default async function AdminDashboard() {
  ...
  const valueStale = lastValueAt ? Date.now() - new Date(lastValueAt).getTime() > 12 * 3600_000 : true;
  ...
  return (
    <main className="mx-auto max-w-5xl px-6 py-10 text-[color:var(--text)]">
      ...
      {(valueStale || (pendingReviews ?? 0) > 0) && (
        <div className="mt-5 grid gap-2">
          {valueStale && (
            <div className="rounded-xl px-4 py-3 text-[13px]" style={{ background: "rgba(251,113,133,0.10)", border: "1px solid rgba(251,113,133,0.35)", color: "#FCA5B6" }}>
              <span className="font-semibold">Values may be stale</span> — last recorded value {ago(lastValueAt)}.
              The Elvebredd scraper runs every 6h; check the tracker repo&apos;s Actions tab if this keeps growing.
            </div>
          )}
          {(pendingReviews ?? 0) > 0 && (
      ...
      <section className="mt-6 grid gap-3" style={{ gridTemplateColumns: "repeat(auto-fill, minmax(170px, 1fr))" }}>
        ...
        <StatCard label="Last value update" value={ago(lastValueAt)} sub={valueStale ? "stale — check scraper" : "scraper healthy"} />
      </section>
